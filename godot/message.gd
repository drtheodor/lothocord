...
static func system_message(text: String) -> Message:
	...
	message.timestamp = Util.get_time_seconds()
...
static func from_json(data: Dictionary) -> Message:
	...
	var _author_name: StringName = author["display_name"] if author.has("display_name") else author["global_name"] if author.has("global_name") else author["username"] # I don't know why this fixes it but it doesl
	...
	# 		if not is_image:
	# 			is_image = Util.is_image(url_attachment)
...
# 		var matches: Array[RegExMatch] = Util.URL_REGEX.search_all(text)
# 		var last_end: int = 0
